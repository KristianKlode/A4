...
int syscall_getpid()
{
  return _syscall(SYSCALL_GETPID, 0, 0, 0);
}

/// Load the file indicated by 'path' as a new process and execute
...
{
  return (int)_syscall(SYSCALL_SPAWN, (uintptr_t)path,
                       (uintptr_t)flags, 0);
}
...
/// Open a new, named semaphore.
/// Arguments: a name for the semaphore and its initial value.
/// Return NULL on error. For instance, if a semaphore with the given name
/// already exists.
sem_t* syscall_sem_open(const char* name, int value)
{
  return (sem_t*)_syscall(SYSCALL_SEM_OPEN, (uintptr_t)name, (uintptr_t)value, 0);
}

/// Destroy the supposedly open semaphore.
/// Return 0 on success, negative on error.
int syscall_sem_close(sem_t *sem)
{
  return (int)_syscall(SYSCALL_SEM_CLOSE, (uintptr_t)sem, 0, 0);
}

/// Procure the semaphore.
/// Return 0 on success, negative on error.
int syscall_sem_p(sem_t *sem)
{
  return (int)_syscall(SYSCALL_SEM_P, (uintptr_t)sem, 0, 0);
}

/// Vacate the semaphore.
/// Return 0 on success, negative on error.
int syscall_sem_v(sem_t *sem)
{
  return (int)_syscall(SYSCALL_SEM_V, (uintptr_t)sem, 0, 0);
}
...
/// Close the open file identified by 'fd'. Zero will be returned
/// success, other values indicate errors.
int syscall_close(int fd)
{
  return (int)_syscall(SYSCALL_CLOSE, (uintptr_t)fd, 0, 0);
}


/// Read 'nbytes' from the open file identified by 'fd' into 'buf', starting
/// at the current file position. Returns the number of bytes actually read
/// (e.g. 0 if the file position is at the end of file) or a negative value
/// on error.
int syscall_read(int fd, void *buf, size_t nbytes)
{
  return (int)_syscall(SYSCALL_READ, (uintptr_t)fd,
                       (uintptr_t)buf, (uintptr_t)nbytes);
}

/// Write 'nbytes' from 'buf' to the open file identified by 'fd', starting
/// from the current file position. Returns the number of bytes actually
/// written or a negative value on error.
int syscall_write(int fd, const void *buf, size_t nbytes)
{
  return (int)_syscall(SYSCALL_WRITE, (uintptr_t)fd, (uintptr_t)buf,
                       (uintptr_t)nbytes);
}

/* Set the file position of the open file identified by 'fd'
...
int syscall_seek(int fd, int offset)
{
  return (int)_syscall(SYSCALL_SEEK,
                       (uintptr_t)fd, (uintptr_t)offset, 0);
}
...
int putc(char c)
{
  return syscall_write(FD_STDOUT, &c, 1);
}
...
{
  return syscall_write(FD_STDOUT, s, strlen(s));
}
...
{
  char c = getc_raw();
  syscall_write(FD_STDOUT, &c, 1); /* Echo back at user. */
  return c;
}
