...
int main(void) {
  ...
  retval = syscall_read(FD_STDIN, &buffer, LENGTH);
  if (retval < 0) {
  ...
  retval = syscall_write(FD_STDOUT, to_out, sizeof(to_out) - 1);
  if (retval < 0) {
    printf("syscall_write to stdout failed with %d :-(\n", retval);
    syscall_halt();
  }

  retval = syscall_write(FD_STDERR, to_err, sizeof(to_err) - 1);
  if (retval < 0) {
  ...
  retval = syscall_write(FD_STDIN, to_out, sizeof(to_out) - 1);
  ...
  retval = syscall_read(FD_STDOUT, &buffer, LENGTH);
  if (retval != IO_INVALID_HANDLE) {
    printf("syscall_read from stdout seems to succeed!? %d\n", retval);
    syscall_halt();
  }

  retval = syscall_read(FD_STDERR, &buffer, LENGTH);
  if (retval != IO_INVALID_HANDLE) {
